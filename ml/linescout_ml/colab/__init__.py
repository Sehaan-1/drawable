...
Reproducibility is part of the contract, not an afterthought: the notebook
clones a *pinned commit*, installs from a *pinned requirement set*
(``ml/colab/requirements-colab.txt``), verifies every model file against
``models.lock.json`` before loading it, and writes the commit, the versions, and
the checkpoint digests into the run report *and* the manifest. See
:mod:`linescout_ml.colab.repro` and :mod:`linescout_ml.colab.checkpoints`.

Installing::

    uv sync --frozen --extra dev      # exactly what CI resolves, no GPU needed
...
from linescout_ml.colab.models import (
    MODEL_CARDS,
    DinoEncoder,
    EncoderError,
    ImageEncoder,
...
from linescout_ml.colab.repro import (
    COLAB_MIRROR_URLS,
    COLAB_PIN,
    COLAB_PIN_PLACEHOLDER,
    COLAB_REPO,
    COLAB_REPO_URL,
    RUNTIME_PACKAGES,
    CheckoutError,
